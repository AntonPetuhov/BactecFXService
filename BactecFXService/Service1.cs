        ...
        static void InsertResultToCGM(string InsertRid, string InsertResult)
        {
            ...
            try
            {
                ...
                using (SqlConnection CGMconnection = new SqlConnection(CGMConnectionString))
                {
                    ...
                    //SqlCommand RIDExistCommand = new SqlCommand($"SELECT * FROM LABETT..searchview s WHERE s.rem_rid = '{InsertRid}' AND s.bes_ank_dttm IS NOT NULL", CGMconnection);
                    ...
                    if (IsCultureTest)
                    {
                        // Проверяем, один ли посевный тест в заявке
